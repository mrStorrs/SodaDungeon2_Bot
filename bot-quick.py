...
match_quickBattle = Image("imgs/quickBattle.png")
match_quickSpend = Image("imgs/quickSpend.png")
match_quickSpendCheck = Image("imgs/quickSpendCheck.png")
match_quickEnd = Image("imgs/quickEnd.png")
match_quickLoot = Image("imgs/quickLoot.png")
match_quickExit = Image("imgs/quickExit.png")
...
def findLocationToClick(template, image_gray, screen, key):
        ...
        if max_val >= 0.85:
            ...
            if key is "exit2" and actions[key] is True: #reset
                ...
                logMsg("#-- ~JPH: " + str(janitorsPerHour) +" --#")
...
def checkIfStepComplete(template, key):
    ...
    elif key is "quickSpend":
        result = cv2.matchTemplate(
            image_gray,
            match_quickSpendCheck.match_gray,
            cv2.TM_CCOEFF_NORMED
        )
...
while main.run:
    ...
    for key in actions:
        ...
        if not actions[key]:
            ...
            if key is "quickBattle":
                findLocationToClick(match_quickBattle, image_gray, screen, key)
            if key is "go":
                findLocationToClick(match_Go, image_gray, screen, key)
            if key is "quickSpend":
                findLocationToClick(match_quickSpend, image_gray, screen, key)
            if key is "quickEnd": 
                findLocationToClick(match_quickEnd, image_gray, screen, key)
            if key is "quickLoot":
                findLocationToClick(match_quickLoot, image_gray, screen, key)
            # if key is "go2":
